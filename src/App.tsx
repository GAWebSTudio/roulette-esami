          ...
          {screen === "settings" && (
            <div className="panel-content">
              ...
              <div className="settings-grid">
                ...
                <SettingSwitch title="Voce docente" subtitle="Pronuncia ad alta voce l’argomento estratto dopo countdown, suspense e pausa." value={settings.voiceEnabled} onChange={() => setSettings((prev) => ({ ...prev, voiceEnabled: !prev.voiceEnabled }))} />
                <div className="setting-card"><div><span className="switch-title">Voce disponibile</span><span className="switch-subtitle">Scegli manualmente la voce del dispositivo/browser.</span></div><select className="select-input voice-select" value={settings.selectedVoice} onChange={(e) => setSettings((prev) => ({ ...prev, selectedVoice: e.target.value }))}><option value="">Automatica docente</option>{voices.map((voice) => <option key={`${voice.name}-${voice.lang}`} value={voice.name}>{voice.name} ({voice.lang})</option>)}</select></div>
                <div className="setting-card"><div><span className="switch-title">Stile voce</span><span className="switch-subtitle">Lettura lenta, più grave e autorevole.</span></div><select className="select-input" value={`${settings.voiceRate}-${settings.voicePitch}`} onChange={(e) => { const [voiceRate, voicePitch] = e.target.value.split("-").map(Number); setSettings((prev) => ({ ...prev, voiceRate, voicePitch })); }}><option value="0.72-0.58">Docente solenne</option><option value="0.78-0.68">Quiz TV lento</option><option value="0.86-0.74">Docente standard</option><option value="1-0.9">Chiara naturale</option></select></div>
